...
const char * const BITCOIN_CONF_FILENAME = "kubercoin.conf";
const char * const BITCOIN_PID_FILENAME = "kubercoin.pid";
...
#else
    const char* pszModule = "kubercoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Kubercoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Kubercoin";
#else
    // Unix
    return pathRet / ".kubercoin";
#endif
#endif
}
